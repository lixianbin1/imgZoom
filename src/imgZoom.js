...
class ImgZoomLi {
  ...
  #top = "50%"
  #left = "50%"
  #theme = "day" //主题 day or night

  constructor() {
    if (!ImgZoomLi.imgZoom) {
      ImgZoomLi.imgZoom = this;
    }
    return ImgZoomLi.imgZoom;
  }
  //图片转base64并缓存
  #imageToBase64(imgUrl, callback) {
  ...
  #imgLoad=()=>{
    ...
    images.forEach(img => {
      ...
      if (imgSrc) {
        this.#imageToBase64(imgSrc)
      }
      const dataSrc = img.getAttribute('data-src');
      if (dataSrc) {
        this.#imageToBase64(dataSrc)
  ...
  #start() {
    ...
    document.addEventListener('click',(event)=>{
      ...
      console.log(even)
      if (target.nodeName == "IMG" && this.#inStr(target.className, 'imgZoom')) {
        this.Open(target);
      }
      if (this.#inStr(target.className, 'imgzoom_shadow') || this.#inStr(target.className, 'imgzoom_close')) {
        this.Close();
      }
      if (this.#inStr(target.className, 'imgzoom_resize')) {
        this.resize(event);
      }
      if (this.#inStr(target.className, 'imgzoom_check')) {
        this.ThemeSwitch(null,event);
      }
      if (this.#inStr(target.className, 'imgzoom_imglink')) {
        this.targetImg(event);
      }
    });
  }
  ...
  lastnext(type) {
    ...
    dom.addEventListener('click',()=>{
      ...
      for (let i = 0; i < imgList.length; i++) {
        if (type == 'next') {
          if(imgList[i]===this.#CurrentImage && i < imgList.length - 1){
            newImg=imgList[i+1]
          }else if(imgList[i]===this.#CurrentImage && i === imgList.length - 1){
            newImg=imgList[0]
          }
        }else{
          if(imgList[i]===this.#CurrentImage && i > 0){
            newImg=imgList[i-1]
          }else if(imgList[i]===this.#CurrentImage && i === 0){
        ...
        if (newImg) {
          ...
          let imgTitle
          if(this.#CurrentImage?.alt){
            imgTitle = this.#CurrentImage.alt
          }else{
            imgTitle = this.#title
          }
          let Base64 = this.#CurrentImage?.src
          if(this.#cache[newImg.src]){
            Base64 = this.#cache[newImg.src]
          }
          document.getElementById('net').src = Base64;
          this.SetTitle(imgTitle)
          break;
        }
      }
    });
  ...
  Open(event) {
    ...
    if (event && event.src) {
      if (imgBox && imgList.length > 0) {
        ...
        if(this.#cache[event.src]){
          img.src = this.#cache[event.src]
        }else{
          img.src = event.src;
          this.#imgLoad()
        }
      }
      imgBox.style.top = this.#top;
      imgBox.style.left = this.#left;
      imgBox.style.width = this.#width + 'px';
      this.SetTitle(imgTitle)
      this.resize(event)
  ...
  resize(event) {
    ...
    let key = this.#CurrentImage.src
    if(this.#size[key]){
      size(this.#size[key].width,this.#size[key].height);
    }else{
      console.log(555)
      let img = new Image();
      img.src = url;
  ...
  targetImg=(event)=>{
    ...
    if(this.#CurrentImage){
      window.open(this.#CurrentImage?.src)  
    }else{
  ...
  SetOption(obj){
    ...
    obj?.top?this.#top=obj.top:""
    obj?.left?this.#left=obj.left:""
  ...
  init(){
    ...
    return Object.freeze({
      SetOption:(obj)=>{this.SetOption(obj)},
