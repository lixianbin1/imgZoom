...
let imgZoom = (()=>{
  ...
  function open(event) {
    useImg = event
    useImg.alt?title = useImg.alt:title = "滚动滚轮缩放图片"
    if (event && event.src) {
  ...
  function lastnext(type) {
    ...
    dom.addEventListener('click', function () {
      ...
      for (let i = 0; i < imgList.length; i++) {
        ...
        if (newImg) {
          useImg = newImg
          var pElement = document.querySelector('p._imgzoom_title');
          if(newImg.alt){
            pElement.firstChild.textContent = newImg.alt
          }else{
            pElement.firstChild.textContent = "滚动滚轮缩放图片"
          }
          let Base64 = useImg.src
          if(chache[newImg.src]){
            Base64 = chache[newImg.src]
          }
          document.getElementById('net').src = Base64;
          document.getElementById('imgzoom_imglink').href = Base64;
          break;
        }
      }
    });
